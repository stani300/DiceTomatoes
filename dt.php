<?php
	...
	switch ( $action ) {
		case "browse":
			...
			$query = "SELECT m.*, AVG(r.rating) FROM movies AS m JOIN ratings AS r ON r.movie_id=m.id WHERE title = '".$target."' OR title LIKE '".$target."' GROUP BY r.movie_id";
