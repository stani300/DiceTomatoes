    ...
    case "recommendation":
        ...
        // then for each row of data, extract the title and any other info we need
			while( ($row = $query_result->fetch_array(MYSQLI_ASSOC) ) && ( $cnt < 27)  ) {
				$thisYr = (int) $row['relyr'];
				$avgScore = $row['avg_score'];
				$thisLang = $row['language'];
				$thisRun = $row['runtime'];
				$sdat[0]->avgRun = $avgRun;
				if ( ( $thisYr > ($avgYr - 10) ) && ( $thisYr < ($avgYr + 10) ) && ( $thisLang == $mylang ) && ( $avgScore > 4 ) && ( $thisRun < ($avgRun+20) ) && ( $thisRun > ($avgRun-20) ) ) {
    ...
    case "analytics":
        ...
        switch ( $show ) {
            ...
            default:
                ...
                break;
            case "MOVIES_COUNT_BY_RELEASE_YEAR":
                $sdat[0]->show = "NUMBER OF MOVIES RELEASED IN EVERY YEAR";
                if ( $showby == "year" ) {
                    $sdat[0]->showby = "Year";
                    $query = 'SELECT count(*) as ydat, year(release_date) as xdat from movies group by xdat';
                }
                break;
        }
