...
function remoteBrowseSearch() {
    ...
    jstr = JSON.stringify({
        "action": "remoteBrowse",
        "target": $(".select2-browse-search option:selected").text()
    });
    // all packed up, let's go find it
    ajaxJCall("dt.php", jstr, browseListUpdate);
}
...
function moviesCountByReleaseYear () {
    ...
    var jstr = JSON.stringify({
        "action": "analytics",
        "show": "MOVIES_COUNT_BY_RELEASE_YEAR",
        "showby": "year",
        "minYr": "55"
    });

    // all packed up, let's go find it
    ajaxJCall("dt.php", jstr, moviesCountByReleaseYearChartUpdate);
...
function moviesCountByReleaseYearChartUpdate ( dat ) {
    ...
    // update kendoChart
    $("#moviesCountByYearChart").kendoChart({
        title: {
            text: obj[0].show
        },
        legend: {
